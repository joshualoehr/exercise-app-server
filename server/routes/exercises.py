...
def get_exercises(user, workoutId):
    try:
        ...
        return json_response(200, last_updated=user.last_updated, exercises=[exercise.toJSON() for exercise in exercises])
...
def exercise(user, workoutId, id):
    try:
        if request.method == 'GET':
            return get_exercise(user, workoutId, id)
        elif request.method == 'POST':
            return post_exercise(user, workoutId, id)
        elif request.method == 'PUT':
            return put_exercise(user, workoutId, id)
        elif request.method == 'DELETE':
            return delete_exercise(user, workoutId, id)
        else:
            abort(405)
    except Exception as e:
        return json_response(
...
def with_exercise(func):
    def with_exercise_wrapper(user, workoutId, id):
...
def get_exercise(user, workoutId, id):
    ...
    if exercise:
        return json_response(200, last_updated=user.last_updated, exercise=exercise.toJSON())
    else:
        return json_response(404, message='Exercise %d not found' % id)


@with_exercise
def post_exercise(exercise, user, workoutId, id):
    ...
    exercise = Exercise(user.id, **exercise)
    db.session.add(exercise)
    db.session.commit()

    return json_response(201, last_updated=user.last_updated, exercise=exercise.toJSON())


@with_exercise
def put_exercise(exercise, user, workoutId, id):
    existing = Exercise.query.filter_by(
        user_id=user.id,
        workoutId=workoutId,
        id=id
    ...
    return json_response(status, last_updated=user.last_updated, exercise=exercise.toJSON())


def delete_exercise(user, workoutId, id):
    exercise = Exercise.query.filter_by(
        user_id=user.id,
        workoutId=workoutId,
        id=id
